...
PLANTUML_TEMPLATE = """Ты — эксперт по PlantUML диаграммам. Твоя задача — создать PlantUML код для activity diagram.
...
ВАЖНО ДЛЯ ДЕТАЛЬНОСТИ - СХЕМА ДОЛЖНА БЫТЬ ПОЛНОЙ, НО НЕ ИЗБЫТОЧНОЙ:
1. ОБЯЗАТЕЛЬНО используй раздел "Описание процесса" из контекста - это основная информация!
2. Разбей процесс на логические шаги - объединяй связанные действия, но сохраняй важные этапы отдельно
3. Если в контексте упоминается несколько шагов подряд, создай действия для ключевых этапов (не обязательно для каждого мелкого шага)
4. Добавь условия (if/then/else) там, где в процессе есть:
   - Важные проверки (валидация, проверка условий)
   - Значимые выборы (да/нет, успех/ошибка)
   - Ключевые ветвления логики
5. Используй альтернативные потоки (else) для важных ветвлений
6. Включи основные действия из "Описание процесса" - не нужно включать каждый мелкий шаг
7. Покажи процесс достаточно детально, но без избыточной детализации
8. Если в контексте есть важные бизнес-правила, которые влияют на процесс, отрази их в условиях
9. Добавь действия для обработки ошибок, если они критичны для процесса
10. Используй fork/endfork для параллельных процессов, если они есть

ПРИМЕР СХЕМЫ (сбалансированная детализация, 6-10 ключевых действий):

...
> **💡 Рекомендация:** Если вы используете Docker (см. раздел "Запуск приложения"), вам не нужно устанавливать Python, Java и настраивать зависимости вручную — все уже настроено в контейнере!

### Необходимое ПО

1. **Python 3.10+** (только если НЕ используете Docker)
   ```bash
...
3. **Java 17+** (для рендеринга PlantUML диаграмм, только если НЕ используете Docker)
   - На macOS с Homebrew:
     ```bash
...
### Способ 1: Использование Docker (рекомендуется для кроссплатформенности) 🐳

**Преимущества Docker:**
- ✅ Работает одинаково на Windows, macOS и Linux
- ✅ Не нужно устанавливать Java вручную
- ✅ Не нужно настраивать Python окружение
- ✅ Все зависимости уже настроены

**Требования:**
- Установите [Docker Desktop](https://www.docker.com/products/docker-desktop/) (бесплатно, ~500MB)
  - Windows: скачайте с сайта Docker
  - macOS: скачайте с сайта Docker или через Homebrew: `brew install --cask docker`
  - Linux: установите Docker Engine: `sudo apt-get install docker.io docker-compose`

**Запуск:**

1. **Убедитесь, что Ollama запущен** (если используете Ollama локально):
   ```bash
   # На Windows/Mac/Linux запустите Ollama
   ollama serve
   # Или скачайте модель:
   ollama pull gemma:latest
   ```

2. **Запустите приложение через Docker (с автоматической очисткой):**
   ```bash
   ...
   **Что делает скрипт:**
   - 🧹 Автоматически очищает неиспользуемые Docker образы, контейнеры, volumes и build cache перед запуском
   - 📦 Останавливает старые контейнеры проекта
   - 🚀 Запускает проект с пересборкой

   **Альтернатива (без автоматической очистки):**
   ```bash
...
# Пересобрать контейнер после изменений (без очистки)
docker-compose up --build
...
# Очистить все (включая образы) вручную
docker system prune -af --volumes
```

**Примечание:** Если Ollama запущен на хосте (не в Docker), приложение автоматически подключится к нему через `host.docker.internal:11434`. Если Ollama тоже в Docker, раскомментируйте секцию `ollama` в `docker-compose.yml`.

### Способ 2: Использование скрипта (локальная установка)
...
- **Если используете Docker**: Проверьте, что Docker Desktop запущен и контейнер собран: `docker-compose up --build`
...
- **Если используете Docker**: Убедитесь, что Ollama доступен. На Windows/Mac используйте `host.docker.internal:11434`, на Linux может потребоваться `network_mode: host` в docker-compose.yml
...
- **На Linux**: Если ошибка `libharfbuzz.so.0: cannot open shared object file`, установите:
  ...
  sudo apt-get install openjdk-17-jre-headless
  ```
- **Если используете Docker**: Java уже установлена в контейнере, проверьте логи: `docker-compose logs ai-business-analyst`
...
- **Контейнер не запускается**: 
  ...
  - Попробуйте пересобрать: `docker-compose up --build --force-recreate`

- **Порт 8501 занят**:
  - Измените порт в `docker-compose.yml`: `"8502:8501"`
  - Или остановите другой процесс на порту 8501

- **Ollama недоступен из контейнера**:
  - На Windows/Mac: используйте `host.docker.internal:11434` (уже настроено)
...
- **Изменения в коде не применяются**:
  - Убедитесь, что volume монтируется: `./ai_ba_agent:/app` в docker-compose.yml
